...
class JournalReportCustomHandler(models.AbstractModel):
    ...
    def _query_aml(self, options, offset=0, journal=False):
        ...
        acc_name_str = f"COALESCE(acc.name->>'{lang}', acc.name->>'en_US')" if \
            self.pool['account.account'].name.translate else 'acc.name'
        j_name_str = f"COALESCE(j.name->>'{lang}', j.name->>'en_US')" if \
            self.pool['account.journal'].name.translate else 'j.name'
        tax_name_str = f"COALESCE(tax.name->>'{lang}', tax.name->>'en_US')" if \
        ...
        for column_group_key, options_group in report._split_options_per_column_group(options).items():
            ...
            query_obj = report._get_report_query(options_group, 'strict_range', domain=[('journal_id', '=', journal.id)])
            ...
            if currency_dif == self.env.company.currency_id.symbol:
                ...
                """,
                    column_group_key=column_group_key,
                    acc_name=SQL(acc_name_str),
                    j_name=SQL(j_name_str),
                    tax_name=SQL(tax_name_str),
        ...
        rslt = {}
        query = SQL(" UNION ALL ").join([SQL("(%(q)s)", q=q) for q in queries])
        
        if offset:
            query = SQL("%(query)s OFFSET %(offset)s", query=query, offset=offset)
        
        # Original code has LIMIT %s at the end of each subquery, 
        # but here we apply it at the end of the UNION ALL if needed.
        # Actually, Odoo's original code had LIMIT in EACH subquery.
        # I'll stick to Odoo's behavior but using UNION ALL wrapper.
    ...
    def _get_tax_grids_summary(self, options, data):
        ...
        tables_sql, where_sql = report._dual_currency_query_get(tax_report_options, 'strict_range')
        lang = self.env.user.lang or get_lang(self.env).code
