...
const AbstractReceiptScreenNew = (Parent) => class extends PrintingMixin(Parent) {
    ...
    async doPrinting(mode) {
        ...
        if (this.env.pos.config.connection_type === "usb") {
            ...
            this.printViaApi();
        } else if (this.env.pos.config.connection_type === "emulator") {
            this.actionPrint();
        } else {
            this.actionPrint();
        }
    ...
    setLines(char) {
        this.order
            ...
            .forEach((line) => {
                ...
                if (line.discount > 0) {
                    amount = convert(line.get_all_prices(1).priceWithoutTaxBeforeDiscount).split(",");
                }
                ...
                if (line.discount > 0) {
                    ...
                    //completar con ceros a la izquierda 0000
                    discount = discount.padStart(4, "0");
                    this.printerCommands.push("p-" + discount);
                    /*if (line.discount == 100) {
                        ...
                        let amount = convert(line.get_price_without_tax()/line.quantity).split(",");
                        amount[0] = this.env.pos.config.flag_21 === '30' ? amount[0].padStart(15, "0"):amount[0].padStart(7, "0");
                        amount = amount.join("");
                        this.printerCommands.push("q-" + amount);
                    }*/
                }
