...
class StockValuationLayer(models.Model):
    ...
    def _valid_field_parameter(self, field_name, parameter):
        return parameter == 'force_save' or super()._valid_field_parameter(field_name, parameter)
    ...
    def write(self, vals):
        ...
        if not 'unit_cost_usd' in vals and not 'value_usd' in vals:
            if not 'quantity' in vals and self.stock_move_id:
                new_rate = company.currency_id_dif.inverse_rate
                for rec in self:
                ...
                if 'quantity' in vals:
                    if float(vals['quantity']) != 0:
                        ...
                        if float(vals['value']) < 0:
                            vals['value_usd'] = float(vals['value']) / company.currency_id_dif.inverse_rate
