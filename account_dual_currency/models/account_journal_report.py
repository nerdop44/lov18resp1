...
class JournalReportCustomHandler(models.AbstractModel):
    ...
    def _get_journal_initial_balance(self, options, journal_id, date_month=False):
        ...
        for column_group_key, options_group in report._split_options_per_column_group(options).items():
            ...
            tables, where_clause, from_params, where_params = report._dual_currency_query_get(new_options, 'normal', domain=[('journal_id', '=', journal_id)])
    ...
    def _query_aml(self, options, offset=0, journal=False):
        ...
        for column_group_key, options_group in report._split_options_per_column_group(options).items():
            ...
            tables, where_clause, from_params, where_params = report._dual_currency_query_get(options_group, 'strict_range', domain=[('journal_id', '=', journal.id)])
    ...
    def _get_tax_grids_summary(self, options, data):
        ...
        tables, where_clause, from_params, where_params = report._dual_currency_query_get(tax_report_options, 'strict_range')
        lang = self.env.user.lang or get_lang(self.env).code
