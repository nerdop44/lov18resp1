...
class ProductProduct(models.Model):
    ...
    currency_id_dif = fields.Many2one('res.currency', string='Moneda Diferente')
...
# 
    ...
    value_usd_svl = fields.Float( compute_sudo=True)
...
#         groups = self.env['stock.valuation.layer']._read_group(domain, ['product_id'], ['value_usd:sum', 'quantity:sum'])
#         products = self.browse()
#         for group in groups:
#             product = self.browse(group['product_id'][0])
#             value_usd_svl = company_id.currency_id_dif.round(group['value_usd'])
#             avg_cost = value_usd_svl / group['quantity'] if group['quantity'] else 0
#             product.value_usd_svl = value_usd_svl
#             products |= product
#         remaining = (self - products)
#         remaining.value_usd_svl = 0
# 
