...
class AccountBankStatementLine(models.Model):
    ...
    tasa_referencia_statement = fields.Float(string="Tasa Referencia", store=True, 
                                             # default=lambda self: self._get_default_tasa()
                                             )
...
# 
#     def _get_default_tasa(self, fecha=None):
#         for rec in self:
...
# 
#     @api.depends('tasa_referencia_statement', 'amount', 'currency_id_dif_statement', 'statement_id.currency_id_journal', 'currency_id')
#     def _amount_usd(self):
#         for rec in self:
#             if rec.currency_id == rec.currency_id_dif_statement:
...
#                 if 'date' in vals:
#                     domain = [('name', '=', vals['date']), ('currency_id', '=', self.env.company.currency_id_dif.id), ('company_id', '=', self.env.company.id)]
#                     #print(domain)
#                     tasa_fecha = self.env['res.currency.rate'].search(domain)
#                     if tasa_fecha:
...
#                         rec.move_id.tax_today = rec.tasa_referencia_statement
