...
class StockLandedCost(models.Model):
    ...
    currency_id = fields.Many2one(
        ...
        string="Moneda Documento (Landed)",
        required=True,
        related="",
        default=lambda self: self.env.user.company_id.currency_id,
    )
    company_currency_id = fields.Many2one('res.currency', string="Moneda Compañía (Landed)", related='company_id.currency_id')
            ...
            for line in cost.cost_lines:
                ...
                for valuation in cost.valuation_adjustment_lines:
                    ...
                    if valuation.cost_line_id and valuation.cost_line_id.id == line.id:
                        ...
                        tax_today = line.tax_today or line.cost_id.company_id.currency_id_dif.inverse_rate or 1.0
                        if valuation.id not in towrite_dict:
                            towrite_dict[valuation.id] = value
                            towrite_dict_usd[valuation.id] = value / tax_today
                        else:
                            towrite_dict[valuation.id] += value
                            towrite_dict_usd[valuation.id] += value / tax_today
