...
class AccountRetentionLine(models.Model):
    ...
    invoice_amount = fields.Float(
        ...
        store=True,
        readonly=False,
        help="Monto gravado antes del IVA"
    )
    ...
    def _onchange_payment_concept_id_refresh_islr(self):
        """
        Este onchange se encarga de:
         - Cargar la base imponible desde la factura
         - Asignar parámetros del concepto según tipo de persona
         - Calcular y mostrar inmediatamente el monto de retención
        """
        for record in self:
            if not record.move_id or not record.payment_concept_id:
                continue

            # === 1. BASE IMPONIBLE DESDE FACTURA ===
            ...
            # IVA desde líneas contables
            iva_lines = record.move_id.line_ids.filtered(
                lambda line: line.tax_group_id.name == 'IVA'
                and line.account_id.account_type == 'liability_current'
            )
            record.iva_amount = sum(iva_lines.mapped('price_subtotal'))

            # === 2. PARÁMETROS DEL CONCEPTO DE PAGO ===
            partner_type_person = record.move_id.partner_id.type_person_id
            if not partner_type_person:
                raise UserError(_("El socio no tiene un tipo de persona definido."))

            line_match = record.payment_concept_id.line_payment_concept_ids.filtered(
                lambda l: l.type_person_id.id == partner_type_person.id
            )

            if not line_match:
                raise UserError(_(
                    "No se encontró una configuración de tarifa para el tipo de persona '%s' en el concepto seleccionado."
                ) % partner_type_person.name)

            record.related_pay_from = line_match.pay_from or 0.0
            record.related_percentage_tax_base = line_match.percentage_tax_base or 0.0
            record.related_percentage_fees = line_match.tariff_id.percentage or 0.0
            record.related_amount_subtract_fees = line_match.tariff_id.amount_subtract or 0.0
            record.foreign_currency_rate = record.move_id.foreign_rate or 1.0

            # === 3. CÁLCULO INMEDIATO Y VISIBLE DE RETENCIÓN ===
            base = record.invoice_amount
            porcentaje_base = record.related_percentage_tax_base / 100.0
            porcentaje_tarifa = record.related_percentage_fees / 100.0
            monto_resta = record.related_amount_subtract_fees

            retention_amount = (base * porcentaje_base * porcentaje_tarifa) - monto_resta
            record.retention_amount = max(retention_amount, 0.0)

            foreign_retention = (
                record.foreign_invoice_amount * porcentaje_base * porcentaje_tarifa
            ) - monto_resta
            record.foreign_retention_amount = max(foreign_retention, 0.0)
        
    def _clear_retention_calculation(self):
        """Limpia los campos de cálculo de retención."""
        self.update({
            'related_pay_from': 0.0,
            'related_percentage_tax_base': 0.0,
            'related_percentage_fees': 0.0,
            'related_amount_subtract_fees': 0.0,
    ...
    def _compute_related_fields(self):
        for record in self:
            ...
            # Asignar montos básicos SIEMPRE, incluso sin payment_concept_id
            if not record.invoice_amount:
                record.invoice_amount = amount_untaxed
            if not record.foreign_invoice_amount:
            ...
            # Solo calcular tasas y conceptos si hay payment_concept_id
    ...
    def _compute_amounts(self):
        #def _compute_amounts(self):
         for line in self:
             if not line.invoice_amount and line.foreign_invoice_amount and line.foreign_currency_rate:
...
#    )
    @api.onchange("payment_concept_id", "invoice_amount", "related_percentage_tax_base", "related_percentage_fees", "related_amount_subtract_fees")
    def _onchange_calculate_retention_amount(self):
        """
        Calcula el monto de retención en tiempo real en el formulario.
        Este onchange se dispara al cambiar cualquier parámetro clave.
        """
        for record in self:
            if not record.payment_concept_id or not record.invoice_amount:
            ...
            base = record.invoice_amount
            porcentaje_base = line_match.percentage_tax_base / 100.0
            porcentaje_tarifa = (line_match.tariff_id.percentage or 0.0) / 100.0
            monto_resta = line_match.tariff_id.amount_subtract or 0.0

            # Cálculo local
            retention_local = (base * porcentaje_base * porcentaje_tarifa) - monto_resta
            record.retention_amount = max(retention_local, 0.0)

            # Moneda extranjera
            foreign_base = record.foreign_invoice_amount or base
            retention_foreign = (foreign_base * porcentaje_base * porcentaje_tarifa) - monto_resta
            record.foreign_retention_amount = max(retention_foreign, 0.0)
 ...
 #       "move_id",
    ...
    def _compute_retention_amount(self):
        """
        Este método es necesario para campos calculados en otros modelos (como totales).
        No debe usarse para UI, pero sí para almacenamiento y reportes.
        """
        base_currency_is_vef = self.env.company.currency_id == self.env.ref("base.VEF")
        ...
        for record in self:
            ...
            foreign_base = record.foreign_invoice_amount or base
            foreign_retention = (
                foreign_base * porcentaje_base * porcentaje_tarifa
            ) - monto_resta
            record.foreign_retention_amount = max(foreign_retention, 0.0)
#    def _compute_retention_amount(self):
#        """
